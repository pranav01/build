...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^radium_") ; then
       RADIUM_BUILD=$(echo -n $1 | sed -e 's/^radium_//g')
       export BUILD_NUMBER=$((date +%s%N ; echo $RADIUM_BUILD; hostname) | openssl sha1 | sed -e 's/.*=//g; s/ //g' | cut -c1-10)
    else
       RADIUM_BUILD=
    fi
    export RADIUM_BUILD
...
function print_lunch_menu()
{
    ...
    echo
    if [ "z${RADIUM_DEVICES_ONLY}" != "z" ]; then
...
function breakfast()
{
    ...
    for f in `/bin/ls vendor/radium/vendorsetup.sh 2> /dev/null`
        do
            echo "including $f"
            . $f
        done
...
function lunch()
{
    ...
    if [ $? -ne 0 ]
    then
        # if we can't find a product, try to grab it off the RADIUM github
        T=$(gettop)
        pushd $T > /dev/null
...
{
    if [ "$OUT" ] ; then
        MODVERSION=$(get_build_var RADIUM_VERSION)
        ZIPFILE=radium-$MODVERSION.zip
        ZIPPATH=$OUT/$ZIPFILE
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
    ...
    if (adb shell cat /system/build.prop | grep -q "ro.radium.device=$RADIUM_BUILD");
    then
...
EOF
    ...
    return $?
        ...
        echo "The connected device does not appear to be $RADIUM_BUILD, run away!"
    fi
}
...
function dopush()
{
    ...
    if (adb shell cat /system/build.prop | grep -q "ro.radium.device=$RADIUM_BUILD") || [ "$FORCE_PUSH" == "true" ];
    then
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $RADIUM_FIXUP_COMMON_OUT ]; then
...
# Add completions
check_bash_version && {
    dirs="sdk/bash_completion vendor/radium/bash_completion"
    for dir in $dirs; do
    if [ -d ${dir} ]; then
